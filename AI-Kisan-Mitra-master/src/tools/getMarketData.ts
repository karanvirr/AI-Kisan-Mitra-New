...
export async function getMarketData(
  ...
  const geminiKey: string = geminiApiKey || process.env.NEXT_PUBLIC_GENERATIVE_API_KEY || "";
  ...
  for (const date of datesToFetch) {
    ...
    if (formattedDate === todayFormatted) {
      url = `${TODAY_BASE_URL}?api-key=${MANDI_API_KEY}&format=json&limit=3`;
      isTodayApi = true;
    } else {
      url = `${HISTORICAL_BASE_URL}?api-key=${MANDI_API_KEY}&format=json&limit=3`;
    }

    // Today API uses PascalCase field IDs, Historical uses lowercase
    const f = (key: string) => isTodayApi ? key : key.toLowerCase();

    if (state) {
      url += `&filters[${f("State")}]=${encodeURIComponent(state)}`;
    }
    if (district) {
      url += `&filters[${f("District")}]=${encodeURIComponent(district)}`;
    }
    if (market) {
      url += `&filters[${f("Market")}]=${encodeURIComponent(market)}`;
    }
    if (commodityName) {
      url += `&filters[${f("Commodity")}]=${encodeURIComponent(commodityName)}`;
    }
    ...
    try {
      ...
      if (data.records && data.records.length > 0) {
        // Normalize: today API returns PascalCase, historical returns lowercase
        const normalized: MandiRecord[] = data.records.map((r: any) => ({
          ...
          Commodity_Code: String(r.Commodity_Code ?? r.commodity_code ?? ""),
        }));
        fetchedRecords.push(...normalized);
      }
    } catch (error) {
